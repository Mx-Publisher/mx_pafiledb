 ...
 * -------------------------------------------------------------------------
 * 
 *    $Id: admin_pa_file.php,v 1.17 2005/12/11 16:19:20 jonohlsson Exp $
 */
...
if ( file_exists( './../viewtopic.php' ) )
{
	...
	if ( !empty( $setmodules ) )
	{
		...
		$module['pafileDB_Download']['2_File_manage_title'] = $file;
		return;
	}	
...
else 
{
	...
	if ( !empty( $setmodules ) )
	{
		...
		$file = basename( __FILE__ );
		$module['pafileDB_Download']['2_File_manage_title'] = 'modules/mx_pafiledb/admin/' . $file;
		return;
	}	
...
}elseif ( $mode == 'do_add' && $file_id )
{
	...
	if ( !$mirrors )
	{
		$message = $lang['Fileedited'] . '<br /><br />' . sprintf( $lang['Click_return'], '<a href="' . append_sid( "admin_pa_file.$phpEx" ) . '">', '</a>' ) . '<br /><br />' . sprintf( $lang['Click_return_admin_index'], '<a href="' . append_sid( $mx_root_path . "admin/index.$phpEx?pane=right" ) . '">', '</a>' );
		mx_message_die( GENERAL_MESSAGE, $message );
	}
}elseif ( $mode == 'delete' )
{
...
if ( in_array( $mode, array( '', 'approved', 'broken', 'do_approve', 'do_unapprove', 'delete', 'file_cat', 'all_file' ) ) )
{
	...
	$cat_list .= $pafiledb->generate_jumpbox( 0, 0, array( $cat_id => 1 ), true );
	$cat_list .= '</select>';
	...
	$pafiledb_template->assign_vars( array( 'L_EDIT' => $lang['Edit'],
			...
			'PAGINATION' => generate_pagination( append_sid( "admin_pa_file.$phpEx?mode=$mode&amp;sort_method=$sort_method&amp;sort_order=$sort_order&cat_id=$cat_id" ), $total_files, $pafiledb_config['pagination'], $start ),
...
}elseif ( $mode == 'add' || $mode == 'edit' || $mirrors )
{
	if ( $mode == 'add' )
	{
		...
		$file_cat_list = $pafiledb->generate_jumpbox( 0, 0, '', true );
	...
	else
	{
		...
		$file_cat_list = $pafiledb->generate_jumpbox( 0, 0, array( $file_info['file_catid'] => 1 ), true );
